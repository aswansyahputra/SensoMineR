...
#' 
#' 
#' @param data A list of object, as returned by the \code{\link{format_holos}}
#' function.
#' @param method A string corresponding to the holistic task realized by the
#' subjects during the experiment: "N" for Napping task, "S" for Sorting task,
#' or "SN" for Sorted Napping task.
#' @param axes A length 2 vector specifying the components of the factorial
#' analysis to plot. By default, the first two components are plotted.
#' @param graph A boolean specifying if the graphical outputs of the factorial
#' analysis should be plotted or not. By default, graph = TRUE.
#' @param export.res A boolean specifying if all the graphical outputs should
#' be exported in the working directory or not. By default, export.res = FALSE.
#' NB: If method = "N", setting this argument to TRUE is the only way to access
#' the individual cognitive processes.
#' @return \item{IDsubjects}{A dataframe containing the concordance between the
#' names of the subjects as given in the Holos experiment and their ID.}
#' \item{summary.task}{The summary of the task realized by the subjects with
#' pieces of information such as the number of steps performed by each subject,
#' the duration of the task, etc.} \item{res.FA}{The results of the factorial
#' analysis as returned by the \code{\link{MFA}} function of the
#' \code{\link{FactoMineR}} package for Napping data, by the \code{\link{MCA}}
#' function for Sorting data, and by the \code{\link{HMFA}} function for Sorted
#' Napping data.} \item{datasets}{All the individuals data sets (digit-tracking
#' data of each subject) and panel data sets (merged final configurations and
#' verbalization).}
#' @seealso \code{\link{format_holos}}
#' @references Le, M.T., Brard, M. & Le, S. (2016). Holos: A collaborative
#' environment for similarity-based holistic approaches. Behavior Research
#' Methods.\cr Le, M.T., Husson, F. & Le, S. (2014). Digit-tracking:
#' Interpreting the evolution over time of sensory dimensions of an individual
#' product space issued from Napping and sorted Napping. Food Quality and
#' Preference.
#' @examples
#' 
#' \dontrun{
